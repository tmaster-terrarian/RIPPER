...
public class PlayerController : MonoBehaviour
{
    ...
    Counter recoil = new Counter();
    float maxRecoil = 4;

    Counter firedelay = new Counter();
    ...
    void UpdateMovement()
    {
        ...
        if(isGrounded)
        {
            ...
            velocityY = Mathf.Max(velocityY, 0.0f);
        }
        else
        {
        ...
        velocity = _velX + (worldUp.up * velocityY);
    ...
    void CollisionDetectionFix()
    {
        ...
        int iterations = 10;
        for(int i = 0; i < 360; i += 360/iterations)
        {
        ...
        if(velocityY > 0 && Physics.Raycast(transform.position, worldUp.up, out RaycastHit jumpStopHit, controller.height + 0.06f, groundLayer))
        {
            velocityY = 0;
        }
    }
